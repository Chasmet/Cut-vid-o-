...
app.get("/api/gpt/library", async (req, res) => {
  ...
  try {
    ...
    res.json({ ok: true, library: gptLibraryView(library), remote_schedule_commands: await recentRemoteScheduleCommands() });
  } catch (error) {
...
app.post("/api/gpt/schedule", async (req, res) => {
  ...
  try {
    ...
    if (invalid.length) {
      res.status(409).json({ error: "invalid_video_names", project: realProject.name, invalid_video_names: invalid.map((item) => item.video_name), pending_videos: stats.pendingVideos.map((video: any) => video.name) });
      return;
    }
    const ordered = [...data.publications].sort((a, b) => a.order - b.order).map((item) => preparePublication({ ...item, status: "a_programmer" }));
    const remoteCommand = await enqueueRemoteScheduleCommand(realProject.name, ordered);
    res.status(202).json({ ok: true, project: realProject.name, timezone: data.timezone, remote_command_id: remoteCommand.id, status: remoteCommand.status, device_status: "EN_ATTENTE_APK", device_confirmed: false, publications: ordered, message: "Verifier ensuite le statut. queued ne signifie pas encore programme." });
  } catch (error) {
...
app.get("/api/gpt/schedule/:id", async (req, res) => {
  ...
  try {
    ...
    res.json({ ok: true, remote_command_id: command.id, project: command.project, status: command.status, device_confirmed: command.status !== "queued", acknowledged_at_millis: command.acknowledged_at_millis ?? 0, ack: command.ack ?? null, publication_count: Array.isArray(command.publications) ? command.publications.length : 0 });
  } catch (error) {
...
app.get("/gpt-actions/openapi.json", (_req, res) => {
  res.json({
    openapi: "3.1.0",
    info: { title: "Cut Video Actions", version: "1.0.0", description: "Lit la bibliotheque Cut Video et envoie de vraies commandes de programmation a l application Android." },
    ...
    components: {
      ...
      schemas: {
        Publication: {
          ...
          properties: {
            order: { type: "integer", minimum: 1, maximum: 999 }, video_name: { type: "string", description: "Nom exact renvoye par getCutVideoLibrary." },
            account: { type: "string", enum: ["chknoirshadow", "qg"] }, platform: { type: "string", enum: ["youtube", "tiktok", "instagram", "x"] },
            date: { type: "string", description: "YYYY-MM-DD" }, time: { type: "string", description: "HH:mm" }, visibility: { type: "string", default: "public" },
            title: { type: "string", maxLength: 70 }, description: { type: "string", maxLength: 90 }, hashtags: { type: "array", maxItems: 5, items: { type: "string" } }
          }
        }
      }
    },
    security: [{ bearerAuth: [] }],
    paths: {
      "/api/gpt/library": { get: { operationId: "getCutVideoLibrary", summary: "Lire la bibliotheque Cut Video", description: "Appeler avant toute creation de metas ou programmation. Retourne les vrais dossiers, fichiers, videos en attente et programmations existantes.", responses: { "200": { description: "OK" }, "401": { description: "Non autorise" }, "409": { description: "APK non synchronisee" } } } },
      "/api/gpt/schedule": { post: { operationId: "scheduleCutVideoPublications", summary: "Envoyer une vraie programmation a Cut Video", description: "Cree une commande pour l APK. Une reponse queued signifie EN ATTENTE APK. Verifier le statut ensuite.", requestBody: { required: true, content: { "application/json": { schema: { type: "object", additionalProperties: false, required: ["project", "publications"], properties: { project: { type: "string" }, timezone: { type: "string", default: "Europe/Paris" }, publications: { type: "array", minItems: 1, maxItems: 100, items: { "$ref": "#/components/schemas/Publication" } } } } } } }, responses: { "202": { description: "Commande en attente APK" }, "400": { description: "Requete invalide" }, "401": { description: "Non autorise" }, "409": { description: "Bibliotheque ou noms invalides" } } } },
      "/api/gpt/schedule/{id}": { get: { operationId: "getCutVideoScheduleStatus", summary: "Verifier si l APK a applique la programmation", description: "Dire PROGRAMME uniquement si status vaut applied. partial signifie application partielle et failed signifie echec.", parameters: [{ name: "id", in: "path", required: true, schema: { type: "string" } }], responses: { "200": { description: "Etat courant" }, "401": { description: "Non autorise" }, "404": { description: "Commande introuvable" } } } }
    }
  });
});
...
source = source.replace("remote_schedule_confirmation_required: true,", "remote_schedule_confirmation_required: true,\n  gpt_actions_mobile_bridge: true,\n  gpt_actions_openapi: '/gpt-actions/openapi.json',");
