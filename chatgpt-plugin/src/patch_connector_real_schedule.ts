...
const oldQueue = `    const ordered = [...publications].sort((a, b) => a.order - b.order).map(preparePublication);\n    const pack = await savePack(realProject.name, ordered);\n    const fiche = [`;
const newQueue = `    const ordered = [...publications].sort((a, b) => a.order - b.order).map(preparePublication);\n    const pack = await savePack(realProject.name, ordered);\n    const remoteCommand = await enqueueRemoteScheduleCommandDeduped(realProject.name, ordered);\n    const remoteResult = await waitForRemoteScheduleCommand(remoteCommand.id, 18_000);\n    const remoteLabel = remoteScheduleStatusLabel(remoteResult.status);\n    const fiche = [`;
if (stable.includes(oldQueue)) {
  ...
  throw new Error("MCP 2.5 patch: stable publication handler marker not found");
}
...
if (stable.includes(oldStructured)) {
  ...
  throw new Error("MCP 2.5 patch: stable structured result marker not found");
}
...
console.log("CUTVIDEO_CONNECTOR_REAL_SCHEDULE_PATCH applied: real stable tool scheduling + dedupe + APK acknowledgement");
