...
const replacement = `app.post("/api/library/pair", async (req, res) => {
  ...
  try {
    ...
    // Une réinstallation de l'APK recrée ses SharedPreferences et donc son device_token.
    // On autorise le remplacement uniquement si l'ancienne installation est inactive.
    // Cela évite qu'un vieux token bloque définitivement le téléphone légitime.
