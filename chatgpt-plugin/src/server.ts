...
async function getRedis() {
  if (!redis) {
    throw new Error("Cut Vidéo storage is not configured.");
  }
  if (!redis.isOpen) {
    if (!redisConnecting) {
...
function compactMetadata(title: string, description: string, hashtags: string[]): string {
  ...
  if (length > MAX_METADATA_CHARS) {
    throw new Error(
      `Les métadonnées font ${length} caractères. Réduis le bloc à ${MAX_METADATA_CHARS} caractères maximum.`,
    );
  }
  return text;
}
...
async function savePack(project: string, publications: ReturnType<typeof preparePublication>[]) {
  ...
  return {
    id,
    import_url: `${PUBLIC_BASE_URL}/handoff-pack/${encodeURIComponent(id)}`,
  };
}
...
function createServer(): McpServer {
  ...
  server.registerTool(
    "list_cutvideo_library",
    {
      ...
      description: "Read the complete synchronized Cut Vidéo library before working on a project. It contains every project/folder currently known from the Android app and the number of videos in each project.",
      inputSchema: {},
      annotations: { readOnlyHint: true, destructiveHint: false, openWorldHint: false, idempotentHint: true },
    },
    async () => {
      ...
      if (!library) {
        throw new Error("La bibliothèque Cut Vidéo n'a pas encore été synchronisée. Ouvre l'APK Cut Vidéo une fois.");
      }
      ...
      return {
        ...
        content: [{
          ...
          text: projects.length
            ? projects.map((project) => `${project.name} (${project.video_count})`).join("\n")
  ...
  server.registerTool(
    "get_cutvideo_project",
    {
      ...
      description: "Use this before generating or scheduling metadata for a named project. Returns the real ordered video filenames from the Android library plus existing schedules for that project. Never invent filenames.",
    ...
    async ({ project }) => {
      ...
      if (!found) {
        throw new Error(`Projet introuvable dans Cut Vidéo : ${project}. Utilise list_cutvideo_library pour voir les projets réels.`);
      }
      const schedules = library.schedules.filter((schedule) => normalizeName(schedule.project) === normalizeName(found.name));
      return {
        structuredContent: {
          project: found.name,
          videos: found.videos,
        ...
        content: [{
          type: "text",
          text: found.videos.map((video, index) => `${index + 1}. ${video.name}`).join("\n"),
        }],
      };
    },
  );
  ...
  server.registerTool(
    "prepare_cutvideo_metadata",
    {
      ...
      description: "Prepare metadata for one real Cut Vidéo file. Total title + description + hashtags must be at most 100 characters and use at most 5 hashtags.",
      inputSchema: {
        video_name: z.string().trim().min(1).max(300),
        account: accountSchema,
        platform: platformSchema,
  ...
  server.registerTool(
    "prepare_cutvideo_publication_pack",
    {
      ...
      description: "Main scheduling tool. First read the synchronized project with get_cutvideo_project. Use only real filenames from that project, stop at the user-requested last video, create different metadata for every video (100 characters max), then create one batch import link for the Android app.",
      inputSchema: {
    ...
    async ({ project, timezone, notes, publications }) => {
      ...
      for (const publication of publications) {
        if (!realNames.has(normalizeName(publication.video_name))) {
          throw new Error(`La vidéo ${publication.video_name} n'existe pas dans le projet ${realProject.name}.`);
        }
      }
...
app.listen(PORT, "0.0.0.0", () => console.log(`Cut Vidéo MCP v1.5.0 listening on port ${PORT}`));
